...
gaussian_norm = 1. / np.sqrt( math.erf(np.sqrt(np.pi * 2.)) / np.sqrt(2.) )
...
triangular_norm = 1. / np.sqrt(2. / 3.)
def triangular_shape(dxsq):
    return triangular_norm * (1. - np.sqrt(dxsq))
...
class AnisoSEM:
    ...
    def __init__(self, y, z, uu, vv, ww, uv, L, Dens=100., fsh='quadratic'):
        ...
        if fsh == 'gaussian':
            self.shape = gaussian_shape
        elif fsh == 'quadratic':
            self.shape = quadratic_shape
        elif fsh == 'triangular':
            self.shape = triangular_shape
